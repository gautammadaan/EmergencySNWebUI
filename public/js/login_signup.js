...
function validateUserNameAndPassword() {
	Array.prototype.search = function(str){
		if(str == null)
			...
			for(var i = 0;i < this.length;i ++){str
				if(this[i] == str){
					$('#alert').text(str);
					return 1;
				}
	...
	var forbiddenUserName = new Array(
  ...
  if($('#userName').val().trim().length < 3){
	  $('#alert').text("userName length should not be under 3");
	  $('#alert').show();
	  $('#passport_alert').remove();
	  return false;
  }
  ...
  if($('#passwd').val().trim().length < 4){
	  $('#alert').text("password length should not be under 4");
	  $('#alert').show();
	  $('#passport_alert').remove();
	  return false;
  }
  
  if(forbiddenUserName.search($('#userName').val().trim()) == 1){
	  $('#alert').text("user name cannot be used");
	  $('#alert').show();
	  $('#passport_alert').remove();
	  return false;
  }
  ...
  if($('#profession').val().trim() == "") {
	    $('#alert').text('Please provide your profession to continue');
	    $('#alert').show();
	    $('#passport_alert').remove();
	    return false;
	  }
